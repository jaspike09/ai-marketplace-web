...
export default function ItemDetails() {
  ...
  useEffect(() => {
    async function fetchItem() {
      const { data, error } = await supabase.from('inventory').select('*').eq('id', itemId).single();
      if (data) {
        setItem(data);
        if (data.is_sold && data.pending_expires_at) {
          const msLeft = new Date(data.pending_expires_at).getTime() - Date.now();
          if (msLeft > 0) {
            setPurchaseComplete(true);
            setTimeLeft(msLeft);
          } else {
            await supabase.from('inventory').update({ is_sold: false, pending_expires_at: null, logistics_status: 'pending' }).eq('id', itemId);
          }
        }
      }
    }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const remaining = new Date(item.pending_expires_at).getTime() - Date.now();
      
      if (remaining <= 0) {
        clearInterval(interval);
        await supabase.from('inventory').update({ is_sold: false, pending_expires_at: null, logistics_status: 'pending' }).eq('id', itemId);
        setPurchaseComplete(false);
        alert("The 1-hour hold has expired. This item is back on the market.");
      } else {
  ...
  const handleBuyNow = async () => {
    ...
    try {
      const response = await fetch('/api/logistics', {
        ...
        body: JSON.stringify({ itemId, method, title: item.title })
      });
      ...
      if (result.success) {
        ...
        await supabase.from('inventory').update({ 
          is_sold: true, 
          pending_expires_at: oneHourFromNow 
        }).eq('id', itemId);
        
        setItem({ ...item, pending_expires_at: oneHourFromNow });
  ...
  if (!item) return <div className="min-h-screen bg-slate-900 flex items-center justify-center text-rose-500 font-black text-2xl animate-pulse">Initializing...</div>;
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-20">
      <nav className="bg-white px-4 py-4 sticky top-0 z-40 shadow-sm flex items-center gap-4">
        <button onClick={() => router.push('/')} className="text-slate-500 hover:text-slate-900 font-bold">← Back to Grid</button>
      ...
      <div className="max-w-5xl mx-auto p-4 md:p-8 flex flex-col md:flex-row gap-8 mt-4">
        ...
        <div className="w-full md:w-1/2 bg-slate-200 rounded-3xl overflow-hidden shadow-lg h-[400px] md:h-[600px] relative">
          ...
          {purchaseComplete && (
            <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm flex flex-col items-center justify-center transition-all">
              <span className="bg-amber-500 text-white font-black text-3xl px-8 py-4 rounded-2xl rotate-[-12deg] shadow-2xl tracking-widest border-2 border-white/20 mb-4">
                PENDING
              </span>
              <span className="text-white font-black text-xl bg-black/70 px-6 py-2 rounded-xl shadow-inner border border-white/10 uppercase tracking-widest">
              ...
              <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                ...
                <div className="space-y-3">
                  <label className={`flex items-center p-4 rounded-2xl border-2 cursor-pointer transition-all ${method === 'pickup' ? 'border-rose-500 bg-rose-50' : 'border-slate-100 hover:border-slate-200'}`}>
                    ...
                    <div className="flex-1"><div className="font-bold text-slate-900">Local Pickup</div><div className="text-sm text-slate-500">AI will text the seller to coordinate a time.</div></div>
                  ...
                  <label className={`flex items-center p-4 rounded-2xl border-2 cursor-pointer transition-all ${method === 'delivery' ? 'border-rose-500 bg-rose-50' : 'border-slate-100 hover:border-slate-200'}`}>
                    ...
                    <div className="flex-1"><div className="font-bold text-slate-900">Local Delivery</div><div className="text-sm text-slate-500">Seller will deliver this directly to you.</div></div>
                  ...
                  <label className={`flex items-center p-4 rounded-2xl border-2 cursor-pointer transition-all ${method === 'shipping' ? 'border-rose-500 bg-rose-50' : 'border-slate-100 hover:border-slate-200'}`}>
                    ...
                    <div className="flex-1"><div className="font-bold text-slate-900">Ship it to me</div><div className="text-sm text-slate-500">AI will calculate dimensions and lowest cost.</div></div>
                    {method === 'shipping' && <div className="w-4 h-4 bg-rose-500 rounded-full shadow-sm" />}
                  </label>
                </div>
              </div>

              <button onClick={handleBuyNow} disabled={isProcessing} className="w-full bg-slate-900 hover:bg-black text-white font-black uppercase tracking-widest py-5 rounded-2xl transition-all shadow-xl hover:shadow-2xl disabled:opacity-50">
            ...
            <div className="space-y-6">
              <div className="bg-emerald-50 border-2 border-emerald-500 p-6 rounded-3xl shadow-lg relative overflow-hidden">
                ...
                <div className="text-emerald-800 font-medium mb-4 space-y-2">
                  <p>Your purchase is secured! Complete the transaction within the time limit or the hold will automatically expire.</p>
                </div>
              </div>

              <button onClick={handleCancelOrder} disabled={isProcessing} className="w-full bg-white text-rose-500 border-2 border-rose-100 hover:border-rose-500 hover:bg-rose-50 font-bold py-4 rounded-2xl transition-all">
